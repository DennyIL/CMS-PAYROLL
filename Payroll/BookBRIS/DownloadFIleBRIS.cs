...
{
    class DownloadFIleBRIS : IStatefulJob
    {
        private static int jobNumber = 0;
        private static String SchCode = "Download FIle BRIS";
        private static String SchName = "Sch Payroll Download FIle BRIS";
        ...
        {
            ILog log = LogManager.GetLogger(typeof(DownloadFIleBRIS));
            try
            {
                if (factory == null)
                {
        ...
        private void Commitjob_transaction(ILog log)
        {
            ...
            IList<TrxPayroll> trxList = session.CreateSQLQuery("select * from trxpayrolls where status = ? and lastupdate<=? and statusbris in (?) order by id ASC")
            ...
            if (trxList.Count > 0)
            {
                foreach (TrxPayroll ts in trxList)
                {
                    ...
                    {
                        log.Info(SchCode + "SeqNum  = " + ts.SeqNumber + " - Download FIle BRIS : Success");
                    }
                    else
                    {
                        int retry = 0;
                        ...
                        if (retry == maxretry)
                        {
                            ...
                            ts.LogFtpBris = DateTime.Now.ToString() + "- Download File BRIS GAGAL|";
                            ts.LastUpdate = DateTime.Now;
                            session.Update(ts);
                            session.Flush();
                            log.Info(SchCode + "SeqNum  = " + ts.SeqNumber + " - Download FIle BriS : Gagal. Akan di retry 10x");
                        }
                    }
                }
            }
