...
{
    class SendFIleBRIS : IStatefulJob
    {
        private static int jobNumber = 0;
        private static String SchCode = "Send FIle BRIS";
        private static String SchName = "Sch Payroll Sen FIle BRIS";
        ...
        {
            ILog log = LogManager.GetLogger(typeof(SendFIleBRIS));
            try
            {
                if (factory == null)
                {
        ...
        private void Commitjob_transaction(ILog log)
        {
            ...
            IList<TrxPayroll> trxList = session.CreateSQLQuery("select * from trxpayrolls where status=? and lastupdate<=? and statusbris = ? order by id ASC")
                ...
                .SetInt32(2, ParameterHelper.TRXSTATUS_PAYROLLNEW_WAITING_FTP_BRIS)//3
            ...
            if (trxList.Count > 0)
            {
                ...
                {
                    if (PayrollHelper.SendFileBRIS(session, ts, log, SchCode))
                    {
                        //udah di handle di helper
                    }
                    else
                    {
                        int retry = 0;
                        ...
                        if (retry == maxretry)
                        {
                            ...
                            session.Update(ts);
                            session.Flush();
                            log.Info(SchCode + "SeqNum  = " + ts.SeqNumber + " - Sending FIle BriS : Gagal. Akan di retry " + maxretry + "x");
                        }
                    }
                }
            }
